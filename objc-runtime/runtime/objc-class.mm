...
static void _class_resolveInstanceMethod(Class cls, SEL sel, id inst)
{
    ...
    {
        // Resolver not implemented.
        /// 没有找到方法 SEL_resolveInstanceMethod 方法实现，就返回
        return;
    }
    /// 类型转换
    BOOL (*msg)(Class, SEL, SEL) = (typeof(msg))objc_msgSend;
    /// 执行实现的 SEL_resolveInstanceMethod 方法
    bool resolved = msg(cls, SEL_resolveInstanceMethod, sel);

    // Cache the result (good or bad) so the resolver doesn't fire next time.
    // +resolveInstanceMethod adds to self a.k.a. cls
    /// 通过 +resolveInstanceMethod 动态添加了方法，再一次进行查找
    IMP imp = lookUpImpOrNil(cls, sel, inst, 
                             NO/*initialize*/, YES/*cache*/, NO/*resolver*/);
...
* 这 2 个方法都不会检查方法是否存在，在 OC 中的表现为，看我们是否实现了 + (BOOL)resolveClassMethod:(SEL)sel或者+ (BOOL)resolveInstanceMethod:(SEL)sel方法
**********************************************************************/

void _class_resolveMethod(Class cls, SEL sel, id inst)
{
    /// 类是否是元类，如果不是调用 _class_resolveInstanceMethod
    if (! cls->isMetaClass()) {
        // try [cls resolveInstanceMethod:sel]
        _class_resolveInstanceMethod(cls, sel, inst);
    } 
    else {
        /// 如果元类,调用 _class_resolveClassMethod 和 resolveInstanceMethod
