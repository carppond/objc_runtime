...
* already tried that.
 ...
 objc_msgSend 调用到这，主要是查找方法，不会涉及到缓存查找，因为之前已经做过了
**********************************************************************/
                       ...
                       bool initialize, bool cache, bool resolver)
{
    ...
    ///无锁状态下，传入的是 NO，所以这里不会执行(汇编那里已经执行过一边查找)
    ...
    /// 检查传入的类是不是已知的类，如果不是抛出异常 'Attempt to use unknown class',如果是通过 NSClassFromString() 获取的可能是未知的类
    checkIsKnownClass(cls);
    
    /// 首先判断类有没有实现，如果没有，先进行实现类
    if (!cls->isRealized()) {
        realizeClass(cls);
    }
    /// 检查类有没有被初始化，如果没有初始化，先进行初始化类
 ...
 retry:    
    ...
    /// 在父类中查找(知道找到根类)，先在缓存中查找，如果找到,且该 imp 不是消息转发的方法，就将该方法返回，并缓存到该类中。如果缓存没有找到，就去父类的方法列表中查找，如果找到，就记录这个方法调用，并缓存该 imp 到该类中，并返回。
    {
             ...
             curClass = curClass->superclass)
        {
            ...
            if (imp) {
                ...
                if (imp != (IMP)_objc_msgForward_impcache) {
                    ...
                    /// 如果是消息转发，先不缓存，也不调用，执行之后 'resolveInstanceMethod' 或者 'resolveClassMethod'
                    break;
                }
            }
            ...
            if (meth) {
                /// 如果在父类的方法列表中找到，就记录这个方法调用，并缓存这个方法到该类中
    ...
    /// 没有找到实现，就尝试调用 _class_resolveMethod，重试一次。其中调用了方法 _class_resolveInstanceMethod 或者 resolveClassMethod 方法去查找
    if (resolver  &&  !triedResolver) {
        runtimeLock.unlock();
        /// 在无锁的状态下，调用_class_resolveMethod
        _class_resolveMethod(cls, sel, inst);
        runtimeLock.lock();
        // Don't cache the result; we don't hold the lock so it may have 
        // changed already. Re-do the search from scratch instead.
        triedResolver = YES;
        /// 不缓存结果，如果已经发生变化，从头开始查找
        goto retry;
    }

    // No implementation found, and method resolver didn't help. 
    // Use forwarding.
    /// 如果还没找到，经过尝试 _class_resolveMethod 也没有得到帮助，就使用消息转发，
